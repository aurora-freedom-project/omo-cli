/**
 * Detects external plugins that may conflict with omo-cli features.
...
/**
 * Known notification plugins that conflict with omo-cli's session-notification.
...
export function getNotificationConflictWarning(pluginName: string): string {
  return `[omo-cli] External notification plugin detected: ${pluginName}

Both omo-cli and ${pluginName} listen to session.idle events.
   Running both simultaneously can cause crashes on Windows.

   omo-cli's session-notification has been auto-disabled.

   To use omo-cli's notifications instead, either:
   1. Remove ${pluginName} from your opencode.json plugins
   2. Or set "notification": { "force_enable": true } in omo-cli.json`
}
