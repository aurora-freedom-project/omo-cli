...
${formatSystemDirective(SystemDirectiveTypes.DELEGATION_REQUIRED)}

You just performed direct file modifications outside \`.opencode/\`.
...
You should NOT:
- Write code directly (except for \`.opencode/\` files like plans and notepads)
- Make direct file edits outside \`.opencode/\`
...
const BOULDER_CONTINUATION_PROMPT = `${formatSystemDirective(SystemDirectiveTypes.BOULDER_CONTINUATION)}
...
- Use the notepad at .opencode/notepads/{PLAN_NAME}/ to record learnings
...
- Files inside \`.opencode/\` (plans, notepads, drafts)
- Reading files for verification
- Running diagnostics/tests

**FORBIDDEN direct file operations:**
- Writing/editing source code
- Creating new files outside \`.opencode/\`
...
${formatSystemDirective(SystemDirectiveTypes.SINGLE_TASK_ONLY)}
...
Update the plan file \`.opencode/tasks/${planName}.yaml\`:
