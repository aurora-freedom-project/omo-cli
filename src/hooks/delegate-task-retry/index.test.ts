...
describe("orchestrator-task-retry", () => {
  ...
  describe("detectDelegateTaskError", () => {
    ...
    it("should detect unknown agent error", () => {
      const output = '[ERROR] Unknown agent: "fake-agent". Available agents: explorer, researcher, architect'
  ...
  describe("buildRetryGuidance", () => {
    ...
    it("should provide fix for unknown agent with available list", () => {
      const errorInfo = { 
        ...
        originalOutput: '[ERROR] Unknown agent: "fake". Available agents: explorer, architect' 
      }
      
      const guidance = buildRetryGuidance(errorInfo)
      
      expect(guidance).toContain("explorer")
