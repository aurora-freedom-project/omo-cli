...
Example: \`.opencode/plans/auth-refactor.md\`
...
**Draft Location**: \`.opencode/drafts/{name}.md\`
...
Write(".opencode/drafts/{topic-slug}.md", initialDraftContent)
\`\`\`

**Every Subsequent Response**: Append/update draft with new information.
\`\`\`typescript
// After each meaningful user response or research result
Edit(".opencode/drafts/{topic-slug}.md", updatedContent)
\`\`\`

**Inform User**: Mention draft existence so they can review.
\`\`\`
"I'm recording our discussion in \`.opencode/drafts/{name}.md\` - feel free to review it anytime."
\`\`\`
...
Generate plan to: \`.opencode/plans/{name}.md\`
...
- Screenshots saved to .opencode/evidence/ for visual verification
...
- [ ] 1. [Task Title]
  ...
  **For Frontend/UI changes** (using playwright skill):
  ...
  7. Screenshot: .opencode/evidence/task-1-login-success.png
  \\\`\\\`\\\`
  ...
  **Evidence to Capture:**
  ...
  - [ ] Screenshot files in .opencode/evidence/ for UI changes
...
Bash("rm .opencode/drafts/{name}.md")
\`\`\`
...
- Keeps .opencode/drafts/ clean for next planning session

### 2. Guide User to Start Execution

\`\`\`
Plan saved to: .opencode/plans/{plan-name}.md
Draft cleaned up: .opencode/drafts/{name}.md (deleted)
...
- You CAN ONLY: ask questions, research, write .opencode/*.md files
