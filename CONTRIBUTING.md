...
First off, thanks for taking the time to contribute! This document provides guidelines and instructions for contributing to omo-cli.
...
git clone https://github.com/aurora-freedom-project/omo-cli.git
cd omo-cli
...
2. **Update your OpenCode config** (`~/.config/opencode/opencode.json` or `opencode.jsonc`):
   ...
   {
     "plugin": [
       "file:///absolute/path/to/omo-cli/dist/index.js"
     ]
   }
   ```
   
   For example, if your project is at `/Users/yourname/projects/omo-cli`:
   ```json
   {
     "plugin": [
       "file:///Users/yourname/projects/omo-cli/dist/index.js"
     ]
   }
   ```

   > **Note**: Remove `"omo-cli"` from the plugin array if it exists, to avoid conflicts with the npm version.
