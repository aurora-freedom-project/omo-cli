...
- **MCP tuyển chọn**: Exa (Web Search), Context7 (Tài liệu real-time), Grep.app (Tìm kiếm code GitHub), AgentQL (Trích xuất dữ liệu web)
- **1243+ Skill đi kèm**: Skill cấp chuyên gia, lưu tập trung tại `~/.config/_skills_/` (symlink tự động từ `~/.opencode/skills/`)
- **Pipeline Task**: Thực thi DAG đa giai đoạn — chuỗi agent tuần tự (analyst → architect → coder → reviewer)
- **10 Lệnh Design**: `/design-audit`, `/design-polish`, `/design-critique`, và 7 lệnh steering khác
...
│   ├── fix-test-types.ts         # Auto-fix tsc errors trong test files
...
│   │   ├── reasoning-bank/          # Học pattern & theo dõi trajectory
│   │   ├── perf-benchmark/           # Benchmark hiệu năng (p50/p95/p99)
│   │   ├── claim-release/            # Khóa tài nguyên độc quyền
│   │   └── workflow-unifier/         # Khám phá & hợp nhất WORKFLOW.md
│   ├── mcp/                      # 🌐 Cấu hình MCP server
│   │   ├── agentql.ts            # Trích xuất dữ liệu web (AgentQL)
...
│   ├── shared/                   # Tiện ích dùng chung (57+ module)
...
| `pipeline_task` | Đồng bộ | DAG đa giai đoạn — chuỗi agent tuần tự (analyst → architect → coder → reviewer). Output giai đoạn N truyền vào N+1. |
...
| **Orchestration** | `delegate_task`, `pipeline_task`, `call_omo_agent`, `background_output`, `background_cancel` | Ủy thác, pipeline đa giai đoạn, và quản lý tác vụ nền |
...
omo-cli tích hợp với **Omni Brain** (SurrealDB) để bổ sung hybrid skill search:

| Tính năng | Mô tả |
|-----------|--------|
| **Hybrid Search** | BM25 full-text + Vector embedding + RRF fusion |
| **Graceful Fallback** | Brain không khả dụng → tự chuyển về filesystem |
| **Content Hash** | Reconciliation thông minh — chỉ sync skill thay đổi |
| **Event Sourcing** | Schema `execution_event` cho audit trail |

Cấu hình trong `omo-cli.json`:
```json
{
  "memory": {
    "enabled": true,
...
> **Chế độ**: `"managed"` (auto Docker) hoặc `"external"` (SurrealDB đã chạy sẵn).
...
Tùy chọn:
  ...
  --min-score <số>         Ngưỡng điểm chất lượng tối thiểu (0-100)
  --strict                 Thoát với mã 1 nếu skill nào dưới --min-score (CI/CD gate)

Ví dụ:
  omo-cli scan-skills                              # Quét tiêu chuẩn
  omo-cli scan-skills --min-score 70 --strict      # CI/CD gate: thất bại nếu < 70
  omo-cli scan-skills --details --output bao-cao.json
```
...
Tạo skill mới với template 8 phần và chấm điểm chất lượng tự động.

```
Cú pháp: omo-cli create-skill <tên> [tùy chọn]

Tùy chọn:
  --description <text>   Mô tả skill
  -t, --target <path>    Thư mục đích (mặc định: ~/.config/_skills_)
  -f, --force            Ghi đè skill đã tồn tại
...
SKILL.md được tạo bao gồm 8 phần: Instructions, Context Detection, Examples,
Anti-Patterns, Edge Cases, Quality Checklist, Iteration Notes, và Metadata.
Sau khi tạo, skill tự động được chấm điểm trên 7 chiều chất lượng.
...
omo-cli create-skill my-skill --description "Skill tùy chỉnh"

# Bước 2: Điền 8 phần trong template (sửa SKILL.md)

# Bước 3: Quét bảo mật & chất lượng
omo-cli scan-skills

# Bước 4: Phân loại theo tầng
omo-cli categorize-skills

# Bước 5: Import theo tầng (an toàn → rủi ro dần)
...
| `memory.mode` | `string` | `"managed"` | `"managed"` (auto Docker) hoặc `"external"` (SurrealDB đã chạy) |
