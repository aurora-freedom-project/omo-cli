...
Meet our main agent: Sisyphus (Claude Opus 4.5 Thinking). Below are the tools Sisyphus uses to keep that boulder rolling.

*Everything below is customizable. Take what you want. All features are enabled by default. Battery Included, works out of the box.*

- Sisyphus's Teammates (Curated Agents)
  - Oracle: Architecture, code review, debugging (Claude Opus 4.5 Thinking)
  - Prometheus: Strategic planning, task breakdown (Claude Opus 4.5 Thinking)
  - Librarian: Official documentation, open source implementations, codebase exploration (Minimax M2.1)
  - Explore: Blazingly fast codebase exploration (Minimax M2.1)
  - Multimodal-Looker: Image/PDF analysis (Gemini 3 Pro)
  - Metis: Plan consultant, hidden requirements discovery (Claude Sonnet 4.5 Thinking)
...
- Curated MCPs:
  - Exa (Web Search)
  - **Context7** (Real-time Documentation Lookup & Versioning)
  - Grep.app (GitHub Code Search)
- **Multilingual Support**: Fully localized in English, Vietnamese, Korean, Japanese, and Chinese.
- Interactive Terminal Supported - Tmux Integration
- Async Agents
- **ClaudeKit-Inspired Enhancements**:
  - **Coding Level (1-10)**: Control response verbosity from terse (1-3) to educational (7-10)
  - **Privacy Awareness**: Auto-warns when accessing sensitive files (.env, *.key, *.pem)
  - **Mental Models**: Prometheus uses Decomposition, 5 Whys, 80/20 Rule, Second-Order Thinking
  - **Skills**: `/watzup` (project status), `/docs` (documentation management)
- ...
...
- **MCPs**: Built-in websearch (Exa), context7 (docs, real-time lookup), grep_app (GitHub search)
...
**OMO Integration:**
Each skill is enhanced with OMO metadata:
```yaml
agents:     # Which agents should use this skill
  - prometheus
  - sisyphus
