...
- [OMO CLI (Oh My OpenCode)](#omo-cli-oh-my-opencode)
  ...
  - [Hệ thống Hook (35+)](#hệ-thống-hook-35)
...
│   │   ├── coder.ts              # Coder — chuyên code (system prompt)
│   │   ├── explorer.ts           # Explorer — duyệt codebase
│   │   ├── researcher.ts         # Researcher — nghiên cứu chuyên sâu
│   │   ├── conductor.ts          # Conductor — lập kế hoạch chiến lược
...
OmoCliPlugin(ctx)
 ...
 +--[3] Register 35 hooks         Tuy dieu kien disabled_hooks
...
35 hook chạy xuyên suốt lifecycle, phân loại theo 6 danh mục:
...
event ---------------+--->|       Hook Pipeline         |
                     |    |                             |
tool.execute.before -+    |  35 hooks x 6 danh muc      |
tool.execute.after --+    |                             |
                     |    |  Moi hook co the:            |
